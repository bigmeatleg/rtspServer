...
int httpd_http_file(socket_data *d, const char *param)
{
    ...
    size += snprintf(buf + size, SENDBUF_SIZE - size, "%s: %s\r\n", HTTP_CONTENT_TYPE, httpd_mime_map(ext));
...
int httpd_http_folder(socket_data *d, const char *path)
{
    ...
    size = httpd_reply_head(buf, 200);
    size += snprintf(buf + size, SENDBUF_SIZE - size, "%s: %s\r\n", HTTP_DATE_TIME, httpd_gmtime());
    size += snprintf(buf + size, SENDBUF_SIZE - size, "%s: %s\r\n", HTTP_CONTENT_TYPE, httpd_mime_map("html"));
    size += snprintf(buf + size, SENDBUF_SIZE - size, "%s: %s\r\n", HTTP_CONNECTION, "close");
    strcat(buf + size, "\r\n"); size += 2;
    size = d->set->send(d->sock, buf, size, 0);
...
int httpd_decode_get(socket_data *d, string_reference *fn, string_reference *pa)
{
    char *p, *e, *f1, *f2;
    int ret;
...
int httpd_decode_post(socket_data *d, string_reference *fn, string_reference *pa)
{
    char *p, *e, *f1, *f2;
    int ret;
...
int httpd_data_filter(socket_data *d)
{
    ...
    if(memcmp(d->head, "GET", 3) == 0) {
        switch(httpd_decode_get(d, &fn, &pa)) {
        case 0: {
            httpd_default(d, &fn);
            break; }
        case 1: {
            pa.ref = ""; pa.size = 0;
            httpd_function(d, &fn, &pa);
            break; }
        case 2: {
            httpd_function(d, &fn, &pa);
            break; }
        ...
        switch(httpd_decode_post(d, &fn, &pa)) {
		case 0:
			httpd_default(d, &fn);
			break;
		case 1: 
...
int httpd_error_page(socket_data *d, int code, const char *err)
{
    ...
    size = httpd_reply_head(head, code);
...
void httpd_init()
{
	if (getcwd(root_doc, sizeof(root_doc)) == NULL) {
    ...
    g_set.send = httpd_send;
    g_set.http_filter = httpd_data_filter;
    g_set.error_page = httpd_error_page;
    g_set.load_plugin = httpd_load_plugin;
    g_set.unload_plugin = httpd_unload_plugin;
    g_set.http_file = httpd_http_file;
    g_set.http_folder = httpd_http_folder;
        ...
        for(i = 0; i < g_set.socks->max; i++) {
            ...
            if(FD_ISSET(d->sock, &fdr)) {
                ...
                if(d->size == 0) {
                    ...
                    d->size = httpd_decode_content_size(d);
...
{
    printf("usage: httpd [-bdhp?]\n\n");
    printf("example: httpd -d/var/www/html/cgi-bin/votest.so -p8080\n");
...
int main(int argc, char *argv[])
{
    ...
    while(argc--) {
        ...
        case 'd':   // preload plugins.
            errstr = httpd_load_plugin(argv[argc] + 2);
            if(errstr != NULL)
