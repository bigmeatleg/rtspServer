/* httpdext: move public functions here, to save plugin(dynamic library) size.
 *
 * author: Qin Wei(me@vonger.cn)
 * compile: cc -c httpdext.c -o httpdext.o
 *
 */
...
const char *httpd_mime_map(const char *ext)
{
    uint i, len;
    if(ext == NULL)
...
const char *httpd_code_message(int code)
{
    switch(code) {
    case 200:
        return "OK";
...
int httpd_reply_head(char *d, int code)
{
    int size = 0;
    size += sprintf(d + size, "HTTP/1.1 %d %s\r\n", code, httpd_code_message(code));
    size += sprintf(d + size, "Server: " httpd_NAME "\r\n");
    return size;
}

const char *httpd_gmtime()
{
...
int httpd_uri_parameters(socket_data *d, string_reference *s)
{
    char *end, *p;
    p = strstr(d->head, "\r\n");
    if(p == NULL) {
